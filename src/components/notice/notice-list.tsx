...
const NoticeList = ({
  ...
  return (
    <div className="bg-white md:p-4 mt-4 rounded shadow text-[12px] md:text-[14px]">
      <ul
        className={`grid grid-cols-[0.5fr_2fr_1fr_1fr] ${isAdmin ? 'md:grid-cols-[0.5fr_1fr_3fr_2fr_2fr_2fr]' : 'md:grid-cols-[1fr_3fr_2fr_2fr]'} text-center font-semibold bg-gray-200 p-2 rounded-t`}
      >
        {isAdmin && (
              ...
              {isDeleteOpen && (
                <div className="absolute top-6 w-16 z-20 bg-white border rounded shadow-lg">
                  <button
                    ...
                    onClick={() =>
                      handleAllDeleteClick(deleteList, deleteNotice)
                    }
                  >
                    추방
                  </button>
      ...
      <div className="flex flex-col space-y-2 mt-2">
        {notices &&
          notices.map((notice) => (
            <ul
              key={notice.id}
              className={`relative grid grid-cols-[0.5fr_2fr_1fr_1fr] ${isAdmin ? 'md:grid-cols-[0.5fr_1fr_3fr_2fr_2fr_2fr]' : 'md:grid-cols-[1fr_3fr_2fr_2fr]'} text-center p-2 hover:bg-gray-100 transition-colors duration-200 cursor-pointer`}
                ...
                {isAdmin && (
                  <>
                    <button className="text-blue-100 hover:text-blue-150">
                      <Link href={`/admin/notices/modify/${notice.id}`}>
                        수정
                      </Link>
                    </button>
                    <span>{` / `}</span>
